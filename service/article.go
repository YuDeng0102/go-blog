...
func (articleService *ArticleService) ArticleSearch(info request.ArticleSearch) (list interface{}, total int64, err error) {
	req := search.Request{
		Query: &types.Query{},
	}
	...
	if info.Query != "" {
		boolQuery.Should = []types.Query{
			{Match: map[string]types.MatchQuery{
				"title": {Query: info.Query},
			}},
			{Match: map[string]types.MatchQuery{
				"abstract": {Query: info.Query},
			}},
			{Match: map[string]types.MatchQuery{
				"content": {Query: info.Query},
			}},
			{Match: map[string]types.MatchQuery{
				"keyword": {Query: info.Query},
			}},
		}
	}
	// 根据标签筛选
...
func (articleService *ArticleService) ArticleCategory() ([]database.ArticleCategory, error) {
	var categories []database.ArticleCategory
...
func (articleService *ArticleService) ArticleLike(req request.ArticleLike) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		var num int
		if errors.Is(tx.Where("user_id = ? AND article_id = ?", req.UserID, req.ArticleID).First(&articleLike).Error, gorm.ErrRecordNotFound) {
			if err := tx.Create(&database.ArticleLike{UserID: req.UserID, ArticleID: req.ArticleID}).Error; err != nil {
				return err
			}
			num = 1
		} else {
			if err := tx.Delete(&articleLike).Error; err != nil {
				return err
			}
			num = -1
		}
...
func (articleService *ArticleService) ArticleLikesList(info request.ArticleLikesList) (interface{}, int64, error) {
	where := global.DB.Where("user_id = ?", info.UserID)
	...
	for _, v := range l {
		article, err := articleService.Get(v.ArticleID)
		if err != nil {
			return nil, 0, err
		}
...
func (articleService *ArticleService) ArticleDelete(req request.ArticleDelete) error {
	if len(req.IDs) == 0 {
		return errors.New("the ID list is empty")
	}
	return global.DB.Transaction(func(tx *gorm.DB) error {
		for _, id := range req.IDs {
			...
			if err := utils.ChangeImagesCategory(tx, illustrations, appTypes.Null); err != nil {
				return err
			}
			// TODO 同时删除该文章下的所有评论
		}
...
func (articleService *ArticleService) ArticleUpdate(req request.ArticleUpdate) error {
	...
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		//同时更新文章类别表中的数据
		if err := articleService.UpdateCategoryCount(tx, oldArticle.Category, req.Category); err != nil {
			return err
		}
		//同时更新文章标签表中的数据
		if err := articleService.UpdateTagsCount(tx, oldArticle.Tags, req.Tags); err != nil {
			return err
		}
		// 同时更新图片表中的图片类别
...
func (article *ArticleService) ArticleList(info request.ArticleList) (list interface{}, total int64, err error) {
	req := &search.Request{
		Query: &types.Query{},
	}
	boolQuery := &types.BoolQuery{}
